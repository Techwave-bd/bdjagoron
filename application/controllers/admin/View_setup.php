...
class View_setup extends CI_Controller {
    ...
    public function live_tv(){
        ...
        $data['live_tv'] = json_decode($this->view_setting_model->get_previous_settings('settings', 120));
        $this->load->view('admin/includes/__header',$data);
        $this->load->view('admin/settings/__live_tv',$data);
        $this->load->view('admin/includes/__footer');

    }


    public function save_live_tv() {

        $S_data ['id'] = 120;
        $S_data ['event'] = 'live_tv';

        $names = $this->input->post('channel_name', TRUE) ?: [];
        $urls  = $this->input->post('channel_url', TRUE) ?: [];
        $channels = [];
        ...
        $check_settings_exist = $this->view_setting_model->check_settings_exist('settings', 120);

        if ($check_settings_exist == 0) {
            $this->db->insert('settings', $S_data);
            $this->session->set_flashdata('message','Save Successfully');
            redirect("admin/view_setup/live_tv");
        } else {
